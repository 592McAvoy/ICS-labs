...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
	...
	case A_NONE:;//wait to be completed!!!
    }
    return val;
}
...
stat_t nexti(y64sim_t *sim)
{
 ...
 //   printf("%x%x",icode,ifun);
	...
	//error message might be wrong
	if (!get_byte_val(sim->m, next_pc, &regfun)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
    	}
	regA = GET_REGA(regfun);
	regB = GET_REGB(regfun);
	next_pc++;
//	printf(" %x%x",regA,regB);
    }
...
//	printf(" %lx",imm);
  ...
  //  printf("\n");
	...
	if(!(NORM_REG(regA) && NORM_REG(regB))){
	    err_print("PC = 0x%lx, Invalid instruction address", sim->pc);//error message
	    return STAT_INS;
	}
	doit = cond_doit(sim->cc, icond);
	if(doit){
	...
	val = get_reg_val(sim->r, regA);
	addr = imm + get_reg_val(sim->r, regB);
	set_long_val(sim->m, addr, val);//may need to check valid memory
	sim->pc = next_pc;
	break;
	...
	if(!get_long_val(sim->m, addr, &val)){
	    err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, addr);//error message
            return STAT_ADR;
	}
	...
	long_t oprdA = get_reg_val(sim->r, regA);
	long_t oprdB = get_reg_val(sim->r, regB);
	long_t result = compute_alu(ifun, oprdB, oprdA);
	sim->cc = compute_cc(ifun, oprdB, oprdA, result);
	...
	break;
      ...
      case I_CALL: /* 8:x imm */
	rspPos = get_reg_val(sim->r, REG_RSP);
	rspPos -= 8;
        set_reg_val(sim->r, REG_RSP, rspPos);
	if(!set_long_val(sim->m, rspPos, next_pc)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rspPos);//error message
            return STAT_ADR;
	}
	sim->pc = imm;
	break;
	...
	rspPos = get_reg_val(sim->r, REG_RSP);
	addr = 0;
	get_long_val(sim->m, rspPos, &addr);
	sim->pc = addr;
	rspPos += 8;
	set_reg_val(sim->r, REG_RSP, rspPos);
	break;
	...
	rspPos -= 8;
	set_reg_val(sim->r, REG_RSP, rspPos);
	if(!set_long_val(sim->m, rspPos, val)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rspPos);//error message
            return STAT_ADR;
        }
	sim->pc = next_pc;
